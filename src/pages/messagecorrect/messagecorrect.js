...
import style from './messagecorrect.scss';
...
class FileUpload extends React.Component {
	constructor(props) {
		...
		this.handlePartUpload = this.handlePartUpload.bind(this);
	}
	...
	render() {
		...
		const uploadProp = {
			...
			beforeUpload: (file) => {
				...
				// 兼容性的处理
				let blobSlice = File.prototype.slice || File.prototype.mozSlice || File.prototype.webkitSlice,
					chunkSize = 1024 * 1024, // 切片每次5M
					chunks = Math.ceil(file.size / chunkSize),
					currentChunk = 0, // 当前上传的chunk
					spark = new SparkMD5.ArrayBuffer(),
					// 对arrayBuffer数据进行md5加密，产生一个md5字符串
					chunkFileReader = new window.FileReader(), // 用于计算出每个chunkMd5
					totalFileReader = new window.FileReader() // 用于计算出总文件的fileMd5
				...
				console.log("finished beforeUpload ?");
				...
				function loadNext() {
					...
					this.fileReader.readAsArrayBuffer(blobSlice.call(file, start, end));
				}
...
class MessageCorrect extends React.Component {
	constructor(props) {
		super(props);
	}
	render() {
		return (
			<div>
				<div style={{marginTop:'30px',marginLeft:'5%',marginRight:'5%',alignItems:'center',width:'90%'}}>
				<Card bordered={true} style={{ width: '100%',fontSize:'25px'}}>
					      <div style={{width:'50%',float:'left'}}>
					      	<div style={{marginLeft:'0%',float:'left',position:'absoult',fontWeight:'900'}}>合同信息</div>
								<List className={style.xlSize}>
							      	<List.Item className={style.xlSize}>合同名称 :<span style={{paddingLeft:'50px',fontSize:'16px',lineHeight:'40px'}}><Input placeholder="合同名称"/></span></List.Item>
							      	<List.Item className={style.xlSize}>合同编号 :<span style={{paddingLeft:'50px'}}><Input placeholder="合同编号" /></span></List.Item>
							      	<List.Item className={style.xlSize}>合同单位 :<span style={{paddingLeft:'50px'}}><Input placeholder="合同编号" /></span></List.Item>
							      	<List.Item className={style.xlSize}>合同金额 :<span style={{paddingLeft:'50px'}}><Input placeholder="合同编号" /></span></List.Item>
							      	<List.Item className={style.xlSize}>基站编码 :<span style={{paddingLeft:'50px'}}><Input placeholder="合同编号" /></span></List.Item>
							    </List>
							      	<Button size="large" style={{marginLeft:'10%',background:'green'}}>确认上传</Button>
					      	</div>
